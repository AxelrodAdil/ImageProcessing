...
img = cv2.imread('Girl_with_a_Mandolin.jpg',0)
kernel = np.ones((3,3),np.uint8)

#display the original image
cv2.namedWindow("OrigImage", cv2.WINDOW_NORMAL)
...
#opening, i.e. erosion followed by a dilation. Opening is used for removing internal noise of the obtained image.
opening = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel)
cv2.namedWindow("Opening", cv2.WINDOW_NORMAL)
cv2.imshow('Opening', opening)
cv2.waitKey()

#closing, i.e. dilation followed by erosion. Closing is used for smoothening of contour and fusing of narrow breaks.
